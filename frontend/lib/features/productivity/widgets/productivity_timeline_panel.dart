...
class _ProductivityTimelinePanelState extends State<ProductivityTimelinePanel> {
  ...
  List<TimelineRow> get _rows {
    final visibleItems = applyTimelineFilter(
      _items,
      widget.filter,
      now: DateTime.now(),
    );
  ...
  VoidCallback? _goalOutcomePressed(Goal goal, GoalCheckIn checkIn) {
    return switch (goal.goalType) {
      ...
      GoalType.count => () => _incrementGoalCheckIn(goal, checkIn),
      _ => null,
    };
  }
  ...
  Future<void> _toggleGoalCheckIn(Goal goal, GoalCheckIn checkIn) async {
    ...
    try {
      final outcome = classifyGoalCheckIn(goal, checkIn, now: DateTime.now());
      final completed = outcome == GoalCheckInOutcome.succeeded ? false : true;
      await _goalCheckInRepository.updateCheckIn(
        goal.id,
        checkIn.id,
      ...
      if (mounted) setState(() => _togglingGoalCheckIns.remove(key));
    }
  }

  Future<void> _incrementGoalCheckIn(Goal goal, GoalCheckIn checkIn) async {
    if (goal.goalType != GoalType.count) return;
