...
/// Eight-week area chart for weekly rate trends (goals, trackers, etc.).
class LoggedTrendChart extends StatelessWidget {
  const LoggedTrendChart({
    super.key,
  ...
  final DateTime? entityStartDate;
  final String Function(DateTime date)? formatStartDate;
  final String emptyEntityLabel;
  ...
  String? _emptyStateMessage() {
    if (_hasData.every((value) => !value)) {
      if (entityStartDate != null && formatStartDate != null) {
        return 'No data yet — $emptyEntityLabel only started From '
            '${formatStartDate!(entityStartDate!)}';
      }
...
class _TrendAreaChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var p = 1; p <= points.length; p++) {
      final isGap =
          p == points.length || points[p].index != points[p - 1].index + 1;
      if (!isGap) continue;
  ...
  void _paintSegment(
    ...
    final linePath = Path()
      ..moveTo(segment.first.offset.dx, segment.first.offset.dy);
