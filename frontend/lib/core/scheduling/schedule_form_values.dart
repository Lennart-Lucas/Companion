...
typedef ScheduleFormValues = TaskScheduleFormValues;
typedef ScheduleFormKeys = TaskScheduleFormKeys;
typedef ScheduleMode = TaskScheduleMode;
typedef ScheduleRepeatType = TaskRepeatType;
...
class TaskScheduleFormValues {
  ...
  Map<String, dynamic>? toScheduleCreateJson({
    ...
    if (preferAnchorField || startDate != null) {
      map['start_date'] =
          _dtstartAtScheduleMidnight(
            RecordJsonUtils.dateOnly(anchorDate),
            timezone,
          ).toIso8601String();
    }
    if (endDate != null && rrule == null) {
      map['end_date'] =
          _dtstartAtScheduleMidnight(RecordJsonUtils.dateOnly(endDate!), timezone)
              .toIso8601String();
    }
  ...
  static DateTime? _dateTimeFromValue(dynamic value) =>
      RecordJsonUtils.dateTimeFromJson(value);

  static DateTime? _anchorFromValue(dynamic value) {
    final parsed = _dateTimeFromValue(value);
    return parsed != null ? RecordJsonUtils.dateOnly(parsed) : null;
  }
  ...
  static List<DateTime> _specificDatesFromResponse(dynamic value) {
    ...
    for (final item in value) {
      if (item is Map) {
        ...
        if (parsed != null) dates.add(RecordJsonUtils.dateOnly(parsed));
      } else {
  ...
  static DateTime _dateOnly(DateTime d) => RecordJsonUtils.dateOnly(d);
