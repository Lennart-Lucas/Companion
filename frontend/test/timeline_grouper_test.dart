...
void main() {
  ...
  group('flattenTimelineRows', () {
    ...
    test('inserts bucket row after today header only', () {
      final today = DateTime(2026, 6, 7);
      final tomorrow = DateTime(2026, 6, 8);
      final summary = TaskBucketSummary(
        overdue: const [],
        today: const [],
        unplanned: const [],
        completed: const [],
      ...
      final sections = [
        TimelineDaySection(day: today, items: []),
        TimelineDaySection(day: tomorrow, items: []),
      ];

      final rows = flattenTimelineRows(
        sections,
        taskBucketSummary: summary,
        today: today,
      );

      expect(rows[0], isA<TimelineDateHeaderRow>());
      expect(rows[1], isA<TimelineTaskBucketRow>());
      expect(rows[2], isA<TimelineAddTaskRow>());
      expect(rows[3], isA<TimelineDateHeaderRow>());
      expect(
        rows.whereType<TimelineTaskBucketRow>(),
        hasLength(1),
      );
    });
